...
- **Major version bumps on content-holding apps require a content rebuild** — `vtex.pages-graphql` keys merchant routes, templates, and Site Editor content by `vendor.app@MAJOR.x:template`. A major bump on any app that ships `store/blocks.json`, `store/routes.json`, `store/templates/`, or `store/contentSchemas.json` makes the previously authored content invisible to the resolver under the new major, so the storefront falls back to default `vtex.store-theme` content. There is no developer-side restore path. Always install new majors in a production-flag dev workspace, re-author the merchant content under the new major in that workspace, smoke-test, and only then promote — see `vtex-io-storefront-theme-versioning`.
