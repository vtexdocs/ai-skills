...
description: >
  Apply when deciding whether and how VTEX IO apps should use Master Data v2 for custom data.
  Covers entity boundaries, schema lifecycle, indexing strategy, and when Master Data is the
  right storage mechanism versus another data approach. Use for reviews, wishlists, forms,
  or other custom data modeling decisions in VTEX IO apps.
metadata:
  track: vtex-io
  tags: [vtex-io, masterdata, strategy, schemas, indexing]
  globs:
    - "masterdata/**/*.json"
    - "node/**/*.ts"
  version: "1.0"
  purpose: Decide when Master Data is the right storage strategy and how to model it safely
  applies_to:
    - choosing Master Data for custom data storage
    - defining entity and schema boundaries
    - planning schema lifecycle and indexing
    - reviewing long-term Master Data usage
  excludes:
    - generic backend runtime behavior
    - low-level client integration patterns
    - route security modeling
    - general app settings
  decision_scope:
    - when to use Master Data versus other data mechanisms
    - how to design entity and schema boundaries
    - how to manage indexing and schema lifecycle
