...
description: >
  Apply when defining or changing the contract of a VTEX IO app through manifest.json,
  builder declarations, dependencies, peerDependencies, billingOptions, and app identity.
  Covers how the app declares capabilities and integration boundaries. Use for scaffolding
  apps, splitting responsibilities across apps, or fixing contract-level link and publish issues.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    ...
    - "node/package.json"
  version: "1.0"
  purpose: Decide what a VTEX IO app exposes as contract and which capabilities belong in its manifest
  applies_to:
    - scaffolding new VTEX IO apps
    - defining builders for app capabilities
    - choosing dependencies vs peerDependencies
    - defining billing behavior at the app contract level
    - fixing manifest-level link and publish failures
    - splitting responsibilities across multiple IO apps
  excludes:
    - service runtime behavior and service.json tuning
    - HTTP route design and middleware chains
    - GraphQL schema and resolver implementation
    - frontend rendering and block composition
    - policy and auth enforcement details
  decision_scope:
    - which capabilities belong in this app contract
    - which builders the manifest should declare
    - when to use dependencies vs peerDependencies
    - when to keep or split responsibilities across apps
    - how app contracts compose safely across edition apps and host environments
