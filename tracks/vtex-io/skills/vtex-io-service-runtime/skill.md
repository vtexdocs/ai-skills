...
description: >
  Apply when designing or implementing the runtime structure of a VTEX IO backend app under node/.
  Covers the Service entrypoint, typed context and state, service.json runtime configuration,
  and how routes, events, and GraphQL handlers are registered and executed. Use for structuring
  backend apps, defining runtime boundaries, or fixing execution-model issues in VTEX IO services.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    ...
    - "node/service.json"
  version: "1.0"
  purpose: Decide how a VTEX IO backend app is structured and executed at runtime
  applies_to:
    - creating VTEX IO backend apps
    - defining Service entrypoints
    - configuring service.json runtime behavior
    - wiring routes events and GraphQL into the runtime
    - fixing backend execution-model issues
  excludes:
    - manifest-level app contract decisions
    - client implementation details and external API integration strategy
    - HTTP route design at the handler level
    - event business logic and worker-specific processing flows
    - GraphQL schema and resolver design
  decision_scope:
    - how to structure the Service entrypoint
    - what belongs in service.json versus application code
    - how runtime registration is separated from handler implementation
    - how typed context state and params should be modeled
