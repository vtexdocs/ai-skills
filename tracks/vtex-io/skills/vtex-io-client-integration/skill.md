...
description: >
  Apply when designing or implementing how a VTEX IO backend app integrates with VTEX services
  or external APIs through @vtex/api and @vtex/clients. Covers choosing the correct client type,
  registering clients in IOClients, configuring InstanceOptions, and consuming integrations through
  ctx.clients. Use for custom client design, VTEX Core Commerce integrations, or reviewing backend
  code that should use VTEX IO client patterns instead of raw HTTP libraries.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    ...
    - "node/**/*.ts"
  version: "1.0"
  purpose: Decide how a VTEX IO backend app should integrate with VTEX and external services through clients
  applies_to:
    - creating custom clients for external APIs
    - integrating VTEX IO services with VTEX Core Commerce APIs
    - choosing between native and custom clients
    - registering clients in IOClients
    - reviewing backend integrations that should use ctx.clients
  excludes:
    - manifest-level app contract decisions
    - service.json runtime sizing and route exposure
    - GraphQL schema design
    - HTTP route authorization and security policy modeling
    - frontend data fetching in react or store apps
  decision_scope:
    - which base client type to extend
    - when to use @vtex/clients instead of writing a custom client
    - how to register and configure clients through IOClients
    - what logic belongs inside a client versus handlers or services
