...
description: >
  Apply when implementing localization and translated copy in VTEX IO apps. Covers the
  `messages` builder, `/messages/*.json`, `context.json`, frontend message usage, and how
  VTEX apps integrate with `vtex.messages` for runtime translation. Use for storefront,
  admin, or backend flows that should use VTEX IO message infrastructure instead of hardcoded strings.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    ...
    - "graphql/**/*.ts"
  version: "1.0"
  purpose: Decide how VTEX IO apps should model and consume translated copy through the messages infrastructure
  applies_to:
    - adding translated UI strings to storefront or admin apps
    - configuring the messages builder and translation files
    - reviewing hardcoded copy in VTEX IO apps
    - implementing backend or GraphQL flows that return translated messages
  excludes:
    - general frontend layout and component design
    - route authorization or service runtime tuning
    - generic copywriting or content strategy
  decision_scope:
    - when to use the messages builder instead of hardcoded strings
    - how to organize message keys and translation files
    - how frontend and backend code should consume translated messages
    - when app-specific overrides of `vtex.messages` behavior are relevant
