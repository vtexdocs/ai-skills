...
description: >
  ...
  migrate-content with the `updateThemeIds` mutation, smoke-test, then promote workflow. Use
  for any operation that changes which version of a content-holding app is installed in `master`.
metadata:
  track: vtex-io
  tags:
    - vtex-io
