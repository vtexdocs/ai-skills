...
metadata:
  ...
  globs:
    ...
    - "node/**/*.ts"
  version: "1.0"
  purpose: Choose IO service paths and HTTP caching headers that match data scope and platform edge behavior
  applies_to:
    - configuring routes in service.json
    - exposing HTTP endpoints consumed through the store domain
    - deciding whether responses may be cached at the edge
  excludes:
    - in-process LRU or VBase patterns (see vtex-io-application-performance)
    - GraphQL @cacheControl directives only (see vtex-io-graphql-api)
  decision_scope:
    - public-vs-segment-vs-private-path
    - cache-control-vs-data-classification
