...
description: >
  Apply when designing or modifying a VTEX IO storefront theme app — the app that owns
  `store/blocks.json`, `store/routes.json`, `store/templates/`, `store/contentSchemas.json`,
  and the storefront page tree assembled from `store.home`, `store.product`, `store.search`,
  `store.custom`, and other native page templates. Covers how a theme app extends a base theme,
  declares routes, composes blocks across pages, and how its `store/` files relate to merchant
  Site Editor content. Use for theme scaffolding, custom page routes, theme-level overrides,
  or reviewing whether a change belongs in the theme app, in a component app, or in app settings.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    ...
    - "store/blocks/**/*.json"
  version: "1.0"
  purpose: Decide what belongs in a storefront theme app and how its `store/` files compose Store Framework pages
  applies_to:
    - scaffolding a new storefront theme app
    - adding a custom storefront route
    - composing blocks for `store.home`, `store.product`, `store.search`, or `store.custom`
    - extending a base theme such as `vtex.store-theme`
    - deciding whether a change belongs in the theme, in a component app, or in app settings
  excludes:
    - block registration in component apps (see `vtex-io-render-runtime-and-blocks`)
    - storefront component implementation (see `vtex-io-storefront-react`)
    - theme version lifecycle and rollback (see `vtex-io-storefront-theme-versioning`)
    - i18n and message content (see `vtex-io-messages-and-i18n`)
  decision_scope:
    - what should ship in the theme app vs in a component app
    - how page templates compose blocks
    - how custom routes map to page contexts
    - when to override a base theme block vs when to add a new block
