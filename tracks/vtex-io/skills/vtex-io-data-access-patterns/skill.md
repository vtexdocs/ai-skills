...
description: >
  ...
  use app settings, configuration apps, Master Data, VBase, VTEX core APIs, or external stores,
  and how to avoid duplicating sources of truth or abusing configuration stores for operational
  data. Use for new data flows, caching decisions, refactors, or reviewing suspicious storage
  and access patterns in VTEX IO apps.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  version: "1.0"
  purpose: Decide where data should live in VTEX IO apps and how to access it safely and efficiently
  applies_to:
    - choosing between app settings, configuration apps, Master Data, VBase, core APIs, and external stores
    - designing new read and write flows for business data in IO apps
    - reviewing data-heavy code for source-of-truth anti-patterns
    - deciding when to cache versus reading directly from the authoritative source
  excludes:
    - detailed Master Data schema modeling
    - app settings schema design
    - service runtime sizing, memory, or concurrency tuning
    - policy and auth-token selection
  decision_scope:
    - which storage or backend is appropriate for a given kind of data
    - when to cache data versus read directly from the source
    - when to treat a system as the single source of truth
    - when not to store or duplicate data in IO at all
