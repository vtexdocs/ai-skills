...
description: >
  Apply when designing or implementing HTTP endpoints exposed by a VTEX IO backend service.
  Covers route boundaries, handler structure, middleware composition, request validation, and
  response modeling for service.json routes. Use for webhook endpoints, partner integrations,
...
metadata:
  ...
  globs:
    ...
    - "node/service.json"
  version: "1.0"
  purpose: Decide how a VTEX IO service should expose and implement explicit HTTP route contracts
  applies_to:
    - exposing REST-like endpoints from VTEX IO apps
    - building webhook or callback handlers
    - structuring middleware chains for routes
    - validating requests and shaping responses
  excludes:
    - service runtime sizing and worker configuration
    - GraphQL schema and resolver design
    - event-driven async processing
    - app policy declaration details
  decision_scope:
    - when to expose HTTP instead of GraphQL or events
    - how to structure route handlers and middlewares
    - how to validate requests and shape responses
    - how to keep route boundaries explicit and small
