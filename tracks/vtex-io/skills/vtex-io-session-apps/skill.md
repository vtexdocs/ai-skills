...
description: >
  Apply when building or debugging a VTEX IO session transform app (vtex.session integration).
  Covers namespace ownership, input-vs-output fields, transform ordering (DAG), public-as-input
  vs private-as-read model, cross-namespace propagation, configuration.json contracts, caching
  inside transforms, and frontend session consumption. Use when designing session-derived state
...
metadata:
  ...
  globs:
    ...
    - "**/transform*.ts"
  version: "1.0"
  purpose: Build session transform apps with correct namespace ownership, input propagation, and caching
  applies_to:
    - building a custom session transform app
    - extending the VTEX session with derived fields
    - debugging session namespace conflicts or stale data
    - designing B2B or pricing context that flows through the session
  excludes:
    - general backend service patterns (see vtex-io-service-apps)
    - application performance outside session transforms (see vtex-io-application-performance)
    - GraphQL schema definitions (see vtex-io-graphql-api)
  decision_scope:
    - which namespace to own vs consume
    - public-input-propagation vs direct-output
    - LRU-plus-VBase inside transforms
