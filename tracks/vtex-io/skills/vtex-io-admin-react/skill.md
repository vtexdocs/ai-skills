...
description: >
  Apply when building VTEX IO admin-facing React interfaces under the admin builder. Covers
  VTEX Styleguide and Shoreline usage, admin page composition, data-heavy admin interactions,
  and keeping administrative interfaces consistent with the VTEX Admin environment. Use for
  settings pages, moderation tools, dashboards, or operational UIs inside VTEX Admin.
metadata:
  track: vtex-io
  tags: [vtex-io, admin-react, admin, styleguide, shoreline, react]
  globs:
    - "react/**/*.tsx"
    - "admin/**/*.json"
  version: "1.0"
  purpose: Decide how admin-facing VTEX IO React interfaces should be designed and implemented
  applies_to:
    - building admin pages and tools
    - using VTEX design system components (vtex.styleguide, @vtex/shoreline)
    - structuring data-heavy admin screens
    - reviewing admin UI consistency
  excludes:
    - shopper-facing storefront components
    - render-runtime block registration
    - backend route design
    - policy modeling
  decision_scope:
    - how admin interfaces should use VTEX design systems (Styleguide or Shoreline)
    - how to structure data-heavy admin flows
    - how to keep admin UI aligned with VTEX Admin conventions
