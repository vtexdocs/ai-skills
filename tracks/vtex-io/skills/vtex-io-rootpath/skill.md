...
description: >
  Apply when building VTEX IO apps that must work correctly in multi-binding stores where
  bindings use path prefixes (e.g. store.com/us/, store.com/br/). Covers rootPath extraction
  from x-vtex-root-path header, useRuntime().rootPath in React, URL construction in backends,
  link generation, asset paths, and API route considerations. Use when the app breaks or
  produces wrong URLs in cross-border or multi-binding setups.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    ...
    - "react/**/*.ts"
  version: "1.0"
  purpose: Build IO apps that work correctly under multi-binding path prefixes
  applies_to:
    - building apps for cross-border or multi-binding stores
    - fixing broken URLs or assets in stores with path prefixes
    - constructing links and API URLs that respect the current binding path
  excludes:
    - general backend service patterns (see vtex-io-service-apps)
    - CDN and edge caching (see vtex-io-service-paths-and-cdn)
  decision_scope:
    - when-rootpath-matters-vs-single-binding
    - header-vs-runtime-extraction
