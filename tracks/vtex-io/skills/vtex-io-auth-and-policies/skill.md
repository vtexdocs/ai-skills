...
description: >
  Apply when deciding or implementing permissions and authorization boundaries for VTEX IO apps.
  Covers manifest policies, outbound-access rules, least-privilege design, and how service
  routes or integrations map to explicit permissions. Use for deciding who is authorized to
  call or consume a capability, adding new integrations, exposing protected routes, or
  reviewing app permissions for overreach or missing access.
metadata:
  track: vtex-io
  tags:
    - vtex-io
  ...
  globs:
    ...
    - "node/routes/**/*.ts"
  version: "1.0"
  purpose: Decide which permissions a VTEX IO app should declare and how to keep them minimal and explicit
  applies_to:
    - adding new external integrations
    - exposing routes that require authorization review
    - reviewing missing or overbroad policies
    - mapping route behavior to permission requirements
  excludes:
    - runtime sizing and service execution tuning
    - frontend data access patterns
    - detailed security trust-boundary review
    - general observability and operational guidance
    - choosing which VTEX auth token should back the request
  decision_scope:
    - which policies are required for a given integration
    - how to apply least privilege to outbound access
    - when route behavior requires policy review
    - how to avoid overbroad app permissions
