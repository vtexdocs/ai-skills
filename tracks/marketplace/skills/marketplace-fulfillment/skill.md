...
description: >
  Apply when implementing fulfillment, invoice, or tracking logic for VTEX marketplace seller connectors.
  Covers the External Seller fulfillment protocol: fulfillment simulation (checkout and indexation), order
  placement with reservation id, order dispatch (authorize fulfillment), OMS invoice and tracking APIs,
  and partial invoicing. Use for seller-side services that must answer within the simulation SLA and
  integrate with VTEX marketplace order management.
metadata:
  track: marketplace
  tags:
    - marketplace
  ...
  globs:
    ...
    - "**/invoice/**/*.ts"
  version: "1.0"
  purpose: Implement External Seller protocol endpoints (simulation, orders, fulfill) plus OMS invoice and tracking for VTEX marketplaces
  applies_to:
    - fulfillment simulation at POST /pvt/orderForms/simulation
    - order placement at POST /pvt/orders (reservation / seller order id)
    - authorize fulfillment at POST /pvt/orders/{sellerOrderId}/fulfill
  ...
  decision_scope:
    - simulation-minimal-vs-checkout-context
    - reservation-orderId-vs-marketplaceOrderId
