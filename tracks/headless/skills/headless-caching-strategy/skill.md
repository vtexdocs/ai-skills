...
description: >
  Apply when implementing caching logic, CDN configuration, or performance optimization for a headless VTEX
  storefront. Covers which VTEX APIs can be cached (Intelligent Search, Catalog) versus which must never be
  cached (Checkout, Profile, OMS), stale-while-revalidate patterns, cache invalidation, and BFF-level caching.
  Use for any headless project that needs TTL rules and caching strategy guidance.
metadata:
  track: headless
  tags:
    - caching
  ...
  globs:
    ...
    - "**/*cache*.ts"
  version: "1.0"
  purpose: Classify VTEX APIs by cacheability and implement correct TTL and invalidation strategies
  applies_to:
    - configuring CDN caching for a headless storefront
    - implementing BFF-level caching with stale-while-revalidate
    - deciding which VTEX APIs can vs cannot be cached
    - designing cache invalidation for catalog data changes
  excludes:
    - BFF architecture decisions (see headless-bff-architecture)
    - search API integration details (see headless-intelligent-search)
    - checkout proxy logic (see headless-checkout-proxy)
  decision_scope:
    - cacheable-vs-non-cacheable-api-classification
    - ttl-selection-per-api
    - cache-layer-selection (CDN vs BFF vs Redis)
