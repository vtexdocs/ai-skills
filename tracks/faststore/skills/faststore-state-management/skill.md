...
description: >
  Apply when using FastStore SDK hooks like useCart, useSession, or useSearch, or working with SDK-related
  state files. Covers cart manipulation, session handling, faceted search, and UI state management from
  @faststore/sdk. Use for any interactive ecommerce feature that relies on FastStore's built-in state management.
track: faststore
tags:
  - faststore
  - sdk
