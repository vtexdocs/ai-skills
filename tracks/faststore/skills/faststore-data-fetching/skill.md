...
metadata:
  ...
  globs:
    ...
    - "faststore.config.*"
  version: "1.0"
  purpose: Decide when and how to extend the FastStore GraphQL API and fetch custom data safely
  applies_to:
    - extending VTEX API schema with custom fields
    - integrating third-party data sources
    - adding fields to predefined queries via fragments
    - creating server-side resolvers
  excludes:
    - client-side state management (use faststore-state-management)
    - visual theming (use faststore-theming)
    - component overrides (use faststore-overrides)
  decision_scope:
    - graphql-api-vs-direct-rest
    - vtex-extension-vs-third-party-extension
    - server-fragment-vs-client-fragment
