...
- The file at `<project_root>/cms/schema.json` must not be edited. It's alway regenerated on every `vtex content generate-schema` script run
...
- **Always verify file existence via shell (`ls`) before assuming files exist** — do not trust the Read tool alone, as it may return cached content for deleted files. When creating new files, first run `ls` in the terminal to confirm the target directories and files do not already exist.
...
| [references/search-facets-and-usesearch-api.md](references/search-facets-and-usesearch-api.md.md)              | Search and facets reference, common pitfalls, accessing search state, or toggling filters in PLP/Search custom sections                                                                                                                                                                   |
