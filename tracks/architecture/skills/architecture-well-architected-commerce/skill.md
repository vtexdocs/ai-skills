...
description: >
  ...
  storefront, IO, headless, marketplace, payments, or any other VTEX module. Grounds work in the
  Well-Architected Commerce framework—Technical Foundation (reliability, trust, integrity; security,
  infrastructure, compliance), Future-proof (innovation, simplicity, efficiency; scalable and adaptable
  solutions), and Operational Excellence (accuracy, accountability, data-driven improvement; process and
  customer experience). Routes implementation detail to product tracks (IO caching and paths, Master Data
  strategy, marketplace integrations). Use for solution design, architecture reviews, and RFP-level
  technical structure.
metadata:
  track: architecture
  tags:
  ...
  globs:
    - "**/architecture/**/*.md"
  version: "1.0"
  purpose: Align VTEX solutions with Well-Architected Commerce pillars before implementation
  applies_to:
    - architecture review
    - solution scoping
    - cross-track technical planning
  excludes:
    - low-level implementation of a single product area without architectural tradeoffs
  decision_scope:
    - native-ootb-vs-vtex-io-extension
