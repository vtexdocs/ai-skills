This skill provides guidance for AI agents working with VTEX FastStore Implementation & Customization. Apply these constraints and patterns when assisting developers with apply when deciding, designing, or implementing data fetching with faststore graphql files in src/graphql/ or src/fragments/, or configuring faststore.config. covers api extensions, graphql fragments, server-side and client-side data fetching, and custom resolver patterns. use for integrating custom data sources or extending the faststore graphql schema.
