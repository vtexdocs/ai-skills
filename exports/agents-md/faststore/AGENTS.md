...
| [references/cms-schema-and-section-registration.md](faststore-storefront/references/cms-schema-and-section-registration.md) | VTEX Headless CMS: `cms_component__*.jsonc` shape, `$componentKey` ↔ `index.tsx`, `generate-schema` / `upload-schema` workflow, scopes, and the rule that new CMS sections rely on context or BFF data (no ad-hoc props) |
