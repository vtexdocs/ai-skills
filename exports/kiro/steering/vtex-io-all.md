...
- [`vtex-io-storefront-theme-app`](vtex-io-vtex-io-storefront-theme-app.md) — When the question is what a theme app actually owns (`store/blocks.json`, `store/routes.json`, page templates) and how those files become VBase content.
- [`vtex-io-render-runtime-and-blocks`](vtex-io-vtex-io-render-runtime-and-blocks.md) — When the question is how a block name in `templates.json` resolves to a React component, and why missing blocks under the active major cause render failures.
- [`vtex-io-data-access-patterns`](vtex-io-vtex-io-data-access-patterns.md) — When the question is whether the data should live in VBase at all, or in app settings, Master Data, or another store.
